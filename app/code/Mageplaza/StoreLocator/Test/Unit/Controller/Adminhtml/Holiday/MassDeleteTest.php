...
use Mageplaza\StoreLocator\Controller\Adminhtml\Holiday\MassDelete;
...
class MassDeleteTest extends TestCase
{
    ...
    public function testExecute()
    {
        ...
        $this->messageManagerMock->expects($this->once())->method('addSuccessMessage')
            ->with(__('Holidays has been deleted.'))
            ->willReturnSelf();
    ...
    public function testExecuteWithException()
    {
        ...
        $this->messageManagerMock->expects($this->once())->method('addErrorMessage')
            ->with(__('Something wrong when deleting Holidays. %1', 'Exception'))
            ->willReturnSelf();
        $resultRedirect = $this->getMockBuilder(Redirect::class)
            ->disableOriginalConstructor()->getMock();
        $this->resultFactoryMock->expects($this->once())->method('create')->with(ResultFactory::TYPE_REDIRECT)
