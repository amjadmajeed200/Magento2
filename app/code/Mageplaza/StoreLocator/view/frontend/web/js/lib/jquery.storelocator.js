...
define(["Mageplaza_StoreLocator/js/lib/handlebars.min"], function (Handlebars) {
    (function ($, window, document, undefined) {
        ...
        var $this, map, listTemplate, infowindowTemplate, dataTypeRead, originalOrigin, originalData, originalZoom,
            ...
            prevSelectedMarkerBefore, prevSelectedMarkerAfter, firstRun, reload;
        ...
        $.extend(Plugin.prototype, {
            ...
            htmlGeocode: function () {
                ...
                //Mageplaza Click button to Auto Geocode
                $('#mp-location-icon').on('click', function () {
                ...
                this.geocode = function (request, callbackFunction) {
                    geocoder.geocode(request, function (results, status) {
                        if (status === google.maps.GeocoderStatus.OK) {
                            ...
                            thisObj.showErrorMageplaza(status);
            ...
            /**
             * Mageplaza show error to front end
             * @param status
             */
            showErrorMageplaza: function (status) {
                var errorEl = $('#mpstorelocator-message-error');
                errorEl.show();
                errorEl.html(
                    '<span>' +
                    'Geocode was not successful for the following reason: ' + status + '<br/>' +
                    'To learn how to fix check this link:' + '' +
                    '<a href="https://developers.google.com/maps/faq" target="_blank">https://developers.google.com/maps/faq</a>' +
                    '</span>'
                );
            },
            ...
            directionsRequest: function (origin, locID, map) {
                ...
                if (destination) {
                    ...
                    if ($('.bh-sl-close-directions-container').length === 0) { //Mageplaza fix multiple close icon
            ...
            closeDirections: function () {
                //Mageplaza Show form search when click close direction button
                $('#bh-sl-user-location').show();
            ...
            listClick: function (map, infowindow, storeStart, page) {
                ...
                /**
                 * Mageplaza Set color for status store = closed
                 */
                $('.store-time-open').each(function () {
                    var el = $(this);
                ...
                $(document).on('click.' + pluginName, '.' + _this.settings.locationList + ' li', function () {
                    ...
                    //Mageplaza set zoom when click list
                    map.setZoom(window.zoomConfig);
            ...
            inlineDirections: function (map, origin) {
                ...
                // Open directions
                // $(document).on('click.'+pluginName, '.' + _this.settings.locationList + ' li .loc-directions', function (e) {
                // 	e.preventDefault();
                ...
                //Mageplaza custom direction
                $('.loc-directions').each(function () {
                    var el = $(this);

                    el.on('click', function (e) {
                        e.preventDefault();
                        var locID = el.attr('marker-id');
                        _this.directionsRequest(origin, parseInt(locID), map);

                        $('#bh-sl-user-location').hide();
                        $('.mp-storelocator-list-location').hide();
                        $('.mp-store-info').hide();
                        $('.mp-detail-info').hide();

                        $('.bh-sl-close-icon').on('click', function () {
                            _this.closeDirections();
                        });
                    })
                });
            ...
            processData: function (mappingObject, originPoint, data, page) {
                ...
                var _this = this;
                var i = 0;
                var orig_lat, orig_lng, origin, name, maxDistance, marker, bounds, storeStart, storeNumToShow,
                    myOptions, distError, openMap, infowindow, nearestLoc;
                var taxFilters = {};
                    ...
                    if (_this.isEmptyObject(taxFilters)) {
                        if (_this.settings.maxDistance === true && maxDistance) {
                            if (typeof locationset[0] === 'undefined' || locationset[0].distance > maxDistance) {
                                // _this.notify(_this.settings.distanceErrorAlert + maxDistance + ' ' + distUnit);      //Mageplaza Disable Alert
                            }
                        } else {
                            if (typeof locationset[0] !== 'undefined') {
                                if (_this.settings.distanceAlert !== -1 && locationset[0].distance > _this.settings.distanceAlert) {
                                    // _this.notify(_this.settings.distanceErrorAlert + _this.settings.distanceAlert + ' ' + distUnit);    //Mageplaza Disable Alert
                                    distError = true;
                                }
                            } else {
