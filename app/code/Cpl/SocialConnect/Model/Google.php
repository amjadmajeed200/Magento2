...
use Cpl\SocialConnect\lib\Google\Client as Google_Client;

/**
 * Class Google
 * @package Cpl\SocialConnect\Model
 */
class Google extends \Cpl\SocialConnect\Model\Sociallogin
{
     ...
     * @var string
     */
    protected $requestEndpoint = 'https://www.googleapis.com/oauth2/v1/userinfo?access_token=';
    ...
    public function __construct(
        ...
        \Cpl\SocialConnect\Helper\Data $slHelper,
    ...
    {
        parent::__construct($context, $registry, $storeManager, $store, $encryptor, $customer, $eavConfig, $attribute, $random, $filesystem, $ioFile, $dir, $customerSession, $customerRepository, $customerData, $customerFactory, $accountManagement, $emailNotificationInterface, $resource, $resourceCollection, $slHelper, $data);
        $this->client = $client;
    ...
    public function fetchUserData($response)
    {
        ...
        $data = $userData = [];
        $client = $this->client;
        $client->setClientId($this->applicationId);
        ...
        if($token) {
            ...
            $userDetails = file_get_contents($this->requestEndpoint . $tokenInfo['access_token'], false, stream_context_create($arrContextOptions));
